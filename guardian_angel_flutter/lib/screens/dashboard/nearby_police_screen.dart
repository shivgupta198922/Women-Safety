...
class _NearbyPoliceScreenState extends State<NearbyPoliceScreen> {
  ...
  void _setCustomMarkerIcon() async {
    // Ensure you have an asset at 'assets/icons/police_marker.png'
    // For now, using default marker if asset is not found or configured.
    try {
      policeIcon = await BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(size: Size(48, 48)),
        'assets/icons/police_marker.png',
      );
    } catch (e) {
      print('Error loading police icon: $e. Using default marker.');
      policeIcon = BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed);
    }
    if (mounted) {
      setState(() {});
    }
  ...
  Future<void> _searchNearbyPolice() async {
    ...
    try {
      ...
      if (currentPosition == null) {
        if (mounted) AppUtils.showSnackBar(context, 'Could not get current location.', isError: true);
        return;
      }
      ...
      if (mounted) AppUtils.showSnackBar(context, 'Error searching for police stations: $e', isError: true);
    }
  }
  ...
  Future<void> _launchMaps(double lat, double lng) async {
    ...
    if (await canLaunchUrl(googleMapsUrl)) {
      ...
      if (mounted) AppUtils.showSnackBar(context, 'Could not launch Google Maps.', isError: true);
    }
  }
